...
class AmmoStats {
    ...
    CalculateVanillaArmorClass(penetrationValue) {
        let penTier = 1;
        while (penTier <= 6) {
    ...
    postDBLoad(container) {
        ...
        this.config = jsonc_1.jsonc.parse(node_fs_1.default.readFileSync(path_1.default.resolve(__dirname, "../config/config.jsonc"), "utf-8"));
                ...
                if (stringToAdd.length > 0) // Only add locale data if there's anything to add
                 {
                    ...
                    else if (this.config.MODE.toLowerCase() === "append" && (this.config.addDamage || this.config.addPen || this.config.addEffectArmorLv)) { // Append mode
